...
- [Contributing to Insurance Hub](#contributing-to-insurance-hub)
  ...
  - [Style Guides](#style-guides)
    ...
    - [Branch Names](#branch-names)
      ...
      - [Enforcing Branch Names](#enforcing-branch-names)
    - [Commit Messages](#commit-messages)
      - [Commit Message Format](#commit-message-format)
      - [Examples](#examples-1)
      - [Details](#details)
      - [Why Follow This Convention?](#why-follow-this-convention)
    - [Makefile](#makefile)
      - [Use Modular Includes](#use-modular-includes)
      - [Declare `.PHONY` for Non-File Targets](#declare-phony-for-non-file-targets)
      - [Use Variables for Versions and Paths](#use-variables-for-versions-and-paths)
      - [Provide a Helpful Default or `help` Target](#provide-a-helpful-default-or-help-target)
      - [Group Related Targets With Comments](#group-related-targets-with-comments)
      - [Support Target Composition](#support-target-composition)
      - [Avoid Hardcoded Paths](#avoid-hardcoded-paths)
      - [Minimize Complex Shell Logic in Makefiles](#minimize-complex-shell-logic-in-makefiles)
      - [Document Custom Targets](#document-custom-targets)
      - [Keep it DRY (Don’t Repeat Yourself)](#keep-it-dry-dont-repeat-yourself)
      - [Fail Early When Appropriate](#fail-early-when-appropriate)
...
- `type` — one of the allowed conventional commit types (`feat`, `fix`, `docs`, `chore`, `refactor`,
  `perf`, `test`, `style`, etc.).
- `[issue-<number>]` — the related GitHub issue number in the form `issue-7`. This part is **optional**
  but strongly recommended when the branch corresponds to a specific ticket.
- `<very-short-description>` — a short, lowercase, dash-separated description of the work being done.
...
* **Match your branch type to your commit type**: for example, a branch starting with `feat/` should
...
A Git [pre-push hook](.githooks/pre-push) is used to validate branch names to ensure they follow the
above convention. If the branch name does not comply with the convention, the **push will be blocked** 
until it's renamed.
...
- The **description** should be very short, concise, and in lowercase without a period at the end.
- Use imperative mood (like a command) for the description, e.g., "add logging support" not "added
  logging support."
...
- For **breaking changes**, include `!` after the type, and provide details in the commit body or
...
#### Use Variables for Versions and Paths
...
#### Provide a Helpful Default or `help` Target
...
- Use variables or discovery methods for directories or file lists to improve portability and ease
  changes.
  ```makefile
  SRC_DIR := ./internal
  ```

#### Minimize Complex Shell Logic in Makefiles
