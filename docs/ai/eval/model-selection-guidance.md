...
Time to first token is the delay between submitting the prompt and seeing the first generated token. On a local machine, this metric matters because it reflects startup overhead, model loading behavior, memory placement, and how much of the workload is waiting on slower memory or partial CPU execution. [blogs.nvidia](https://blogs.nvidia.com/blog/ai-decoded-lm-studio/)
